...
impl HasNew for NFTContractMetadata{
    fn new(media_url:String) -> Self {
        let extract_cid = |url: &str| {
        url.split("/ipfs/").nth(1).unwrap_or("").to_string()};
        Self {
...
impl Contract{
    ...
    pub fn mint_nft(&mut self){
        ...
        let token_metadata: TokenMetadata = TokenMetadata {
            ...
            media: Some(self.metadata.get().unwrap().icon.clone().unwrap_or_default()),
            copies: Some(1),
        ...
        self.tokens.internal_mint(caller_id.clone().to_string(), caller_id.clone(), Some(token_metadata));

        self.minters.insert(&caller_id);

    }
    pub fn get_metadata(&self) -> NFTContractMetadata {
        self.metadata.get().unwrap_or_else(|| NFTContractMetadata::new("".to_string()))
    }
